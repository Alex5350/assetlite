...
/**
 * Reports: the full inventory summary (stat cards, per-office and per-category
 * tables) plus direct downloads of the asset register (Excel + PDF).
 */
@Component({
...
export class Reports {
  ...
  protected readonly totals = computed(() => {
    ...
    return {
      ...
      disposed: sum((o) => o.disposedCount),
    };
  });
  ...
  protected readonly statCards = computed<StatCard[]>(() => {
    ...
    return [
      ...
      { label: 'Disposed', value: totals.disposed.toLocaleString(), hint: 'permanently removed', tone: 'retired' },
  ...
  /** Direct download endpoints (GET, no auth) — plain anchors let the browser save the files. */
  protected readonly excelUrl = this.api.registerExcelUrl();
  protected readonly pdfUrl = this.api.registerPdfUrl();
